...
scen_list<-Scenario_LUT[c(1:2, 7:8),1]
...
for (ScenarioName in scen_list)   # for each scenario and replicate, compiles data into a single data frame
{
  for (i in 1:no_reps)
  {
    ...
    log_file <- paste(dir, ScenarioName, "/harvest/", "harvest-summary-log.csv", sep="")
...
# dev.off()
  ...
  output.plot.v1<-ggplot(Thinning_Only, aes(x=Year, y= TotalBiomassHarvested, group = as.factor(Scenarios)))+ theme_classic()+ geom_line(aes(linetype = Scenarios, color = Scenarios), size=1.3) + scale_linetype_manual(values=c("solid", "solid", "solid", "solid", "solid", "solid", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed"))+scale_color_manual(values=plt.cols.short) +scale_y_continuous(limits=c(0,70000))+scale_x_continuous(limits=c(2010,2060))+
