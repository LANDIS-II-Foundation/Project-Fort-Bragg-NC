...
plt.cols.short <- c("#E69F00", "#E69F00", "#56B4E9", "#56B4E9")


#Graph for Rx fire sites for all scenarios
# dev.off()
  png_name<-paste(output_dir, "Rx_Extent_vs_Time_071919.png", sep="")
  output.plot.v1<-ggplot(Final_data, aes(x=Year, y= TotalBurnedSitesRx, group = as.factor(Scenarios)))+ theme_classic()+geom_line(aes(linetype = Scenarios, color = Scenarios), size=1.3) + scale_linetype_manual(values=c("solid", "dashed", "solid", "dashed"))+scale_color_manual(values=plt.cols.short) +scale_y_continuous(limits=c(3000,15000))+scale_x_continuous(limits=c(2010,2060))+
