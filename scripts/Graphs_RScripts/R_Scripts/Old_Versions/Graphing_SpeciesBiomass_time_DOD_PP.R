...
for (Scenario in scen_list)   # for each scenario and replicate, compiles data into a single data frame
{
  for (i in 1:no_reps)
  {
    ...
    log_file <- paste(dir, Scenario, "/", "spp-biomass-log.csv", sep="")
...
all_data2 <- merge(all_data, Year_LUT_50y, by.x="Time", by.y="Time", all=TRUE)
...
# plt.cols.short <- c("orange", "blue", "green","yellow","black",
#                     "grey","purple","black","deeppink1","olivedrab") #Number corresponds to scenarios

plt.cols.short <- c("blue", "orange", "black","gray","darkslateblue","orangered")

#Graph for all scenarios for Loblolly
# dev.off()
  png_name<-paste(output_dir, "LoblollyBiomass_vs_Time_071619.png", sep="")
  output.plot.v1<-ggplot(FortBragg_data, aes(x=Time, y=AboveGroundBiomass_LobPine, color=Scenario)) +  geom_line(size=1.5) + scale_color_manual(values=plt.cols.short)+scale_y_continuous(limits=c(0,10000))+scale_x_continuous((limits=c(0,50)))
  output.plot.v2<-output.plot.v1+theme_classic()+ggtitle("Loblolly Biomass") +theme(plot.title = element_text(hjust=0.5)) + labs(colour="Climate Scenario")+xlab("Time")+ylab("Biomass (g/m2)") + theme(legend.position = "right")
  ...
  # dev.off()
  png_name<-paste(output_dir, "LongleafBiomass_vs_Time_071619.png", sep="")
  output.plot.v1<-ggplot(FortBragg_data, aes(x=Time, y=AboveGroundBiomass_LongleafPine, color=Scenario)) +  geom_line(size=1.5) + scale_color_manual(values=plt.cols.short)+scale_y_continuous(limits=c(0,15000))+scale_x_continuous((limits=c(0,50)))
  output.plot.v2<-output.plot.v1+theme_classic()+ggtitle("Longleaf Biomass") +theme(plot.title = element_text(hjust=0.5)) + labs(colour="Climate Scenario")+xlab("Time")+ylab("Biomass (g/m2)") + theme(legend.position = "right")
  ...
  # dev.off()
  png_name<-paste(output_dir, "SweetGumBiomass_vs_Time_071619.png", sep="")
  output.plot.v1<-ggplot(FortBragg_data, aes(x=Time, y=AboveGroundBiomass_SweetGum, color=Scenario)) +  geom_line(size=1.5) + scale_color_manual(values=plt.cols.short)+scale_y_continuous(limits=c(0,500))+scale_x_continuous((limits=c(0,50)))
  output.plot.v2<-output.plot.v1+theme_classic()+ggtitle("Sweetgum Biomass") +theme(plot.title = element_text(hjust=0.5)) + labs(colour="Climate Scenario")+xlab("Time")+ylab("Biomass (g/m2)") + theme(legend.position = "right")
