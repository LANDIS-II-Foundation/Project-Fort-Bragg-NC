...
# dev.off()
  ...
  output.plot.v1<-ggplot(all_data2, aes(x=Time, y=AGB, color=Scenario)) +  geom_line(size=1.5) + scale_color_manual(values=plt.cols.short)+scale_y_continuous(limits=c(0,30000))+scale_x_continuous((limits=c(0,50)))
  output.plot.v2<-output.plot.v1+theme_classic()+ggtitle("Total Biomass") +theme(plot.title = element_text(hjust=0.5)) + labs(colour="Climate Scenario")+xlab("Time")+ylab("Biomass (g/m2)") + theme(legend.position = "right")
