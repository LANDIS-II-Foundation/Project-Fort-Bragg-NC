...
tail(all_data2)
head(all_data2)
# plt.cols.short <- c("grey", "blue", "green","yellow","orange")
plt.cols.short <- c("orange", "blue","black")

# cbbPalette <- c("#000000", "#E69F00", "#56B4E9")

Thinning_Only<-subset (all_data2, all_data2$Prescription %in% " Thinnning_Pines")
head(Thinning_Only)

#Graph for biomass harvest for all scenarios
# dev.off()
  png_name<-paste(output_dir, "HarvestedBiomass_vs_Time_071619.png", sep="")
  output.plot.v1<-ggplot(Thinning_Only, aes(x=Year, y= TotalBiomassHarvested, color=Scenario)) +  geom_line(size=1.5) + scale_color_manual(values=plt.cols.short)+scale_y_continuous(limits=c(0,70000))+scale_x_continuous((limits=c(0,50)))
  output.plot.v2<-output.plot.v1+theme_classic()+ggtitle("Total Biomass Harvested") +theme(plot.title = element_text(hjust=0.5)) + labs(colour="Climate Scenario")+xlab("Time")+ylab("Biomass (g/m2)") + theme(legend.position = "right")
  ...
  # dev.off()
  png_name<-paste(output_dir, "HarvestedArea_vs_Time_071619.png", sep="")
  output.plot.v1<-ggplot(Thinning_Only, aes(x=Year, y= HarvestedSites, color=Scenario)) +  geom_line(size=1.5) + scale_color_manual(values=plt.cols.short)+scale_y_continuous(limits=c(900,1200))+scale_x_continuous((limits=c(0,50)))
  output.plot.v2<-output.plot.v1+theme_classic()+ggtitle("Total hectares Harvested") +theme(plot.title = element_text(hjust=0.5)) + labs(colour="Climate Scenario")+xlab("Time")+ylab("Biomass (g/m2)") + theme(legend.position = "right")
